erpnext.PointOfSale.PastOrderSummary = class {
	...
	get_discount_html(doc) {
		if (doc.discount_amount || doc.additional_discount_percentage) {
			const discount_type = doc.discount_type || 'Percentage';
			let discount_display = '';
			
			if (discount_type === 'Percentage' && doc.additional_discount_percentage) {
				discount_display = `(${doc.additional_discount_percentage}%)`;
			} else if (discount_type === 'Amount') {
				discount_display = format_currency(doc.discount_amount, doc.currency);
			}
			
			return `<div class="summary-row-wrapper">
						<div>${__("Discount")} ${discount_display}</div>
