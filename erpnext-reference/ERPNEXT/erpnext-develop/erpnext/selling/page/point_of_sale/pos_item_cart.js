erpnext.PointOfSale.ItemCart = class {
	...
	show_discount_control() {
		...
		this.$add_discount_elem.html(`
			<div class="add-discount-wrapper">
				<div class="discount-type-field"></div>
		...
		let discount = discount_type === 'Percentage' ? frm.doc.additional_discount_percentage : frm.doc.discount_amount;

		// Add discount type selector
		this.discount_type_field = frappe.ui.form.make_control({
			df: {
				label: __("Type"),
				fieldtype: "Select",
				...
				onchange: function() {
					me.discount_field.set_value('');
					frappe.model.set_value(frm.doc.doctype, frm.doc.name, 'discount_type', this.value);
					me.discount_field.df.placeholder = this.value === "Percentage" ? 
						__("Enter discount percentage") : __("Enter discount amount");
					me.discount_field.refresh();
				}
			},
			parent: this.$add_discount_elem.find(".discount-type-field"),
			render_input: true,
		});
		...
		this.discount_field = frappe.ui.form.make_control({
			df: {
				...
				onchange: function() {
					if (this.value === '') {
						frappe.model.set_value(frm.doc.doctype, frm.doc.name, 'discount_type', '');
						...
						const discount_type = me.discount_type_field.get_value();
		...
		this.discount_type_field.toggle_label(false);
		this.discount_field.toggle_label(false);
		this.discount_field.set_focus();
	}

	update_discount_fields() {
		const discount_type = this.discount_type_field.get_value();
		const discount_value = flt(this.discount_field.get_value());
		const grand_total = this.frm.doc.grand_total;
		
		if (discount_type === 'Percentage') {
			this.frm.doc.additional_discount_percentage = discount_value;
			this.frm.doc.discount_amount = flt(grand_total * discount_value / 100);
		} else {
			this.frm.doc.discount_amount = discount_value;
			this.frm.doc.additional_discount_percentage = 0;
		}
		
		this.frm.doc.discount_type = discount_type;
		this.calculate_taxes_and_totals();
	}

	calculate_taxes_and_totals() {
		const items = this.frm.doc.items;
		const grand_total = items.reduce((total, item) => total + item.amount, 0);
		const discount_type = this.frm.doc.discount_type || 'Percentage';
		const discount_value = discount_type === 'Percentage' ? 
			this.frm.doc.additional_discount_percentage : 
			this.frm.doc.discount_amount;

		// Calculate net total after discount
		let net_total = grand_total;
		if (discount_type === 'Percentage' && flt(discount_value)) {
			net_total = grand_total - flt(grand_total * discount_value / 100);
		} else if (discount_type === 'Amount' && flt(discount_value)) {
			net_total = grand_total - flt(discount_value);
		}

		this.frm.doc.total = grand_total;
		this.frm.doc.base_total = grand_total;
		this.frm.doc.net_total = net_total;
		this.frm.doc.base_net_total = net_total;
		this.frm.doc.grand_total = net_total;
		this.frm.doc.base_grand_total = net_total;
		this.frm.doc.rounding_adjustment = 0;
		this.frm.doc.rounded_total = net_total;

		this.update_paid_amount_status(false);
	}

	hide_discount_control(value) {
		const discount_type = this.discount_type_field.get_value();
		if (!flt(value)) {
			...
			this.discount_type_field = undefined;
		} else {
			this.$add_discount_elem.css({
				border: "1px dashed var(--dark-green-500)",
				padding: "var(--padding-sm) var(--padding-md)",
			});
			const discount_label = discount_type === 'Percentage' ? `${value}%` : format_currency(value, this.events.get_frm().doc.currency);
			this.$add_discount_elem.html(
				`${this.get_discount_icon()} ${__("Discount Applied")}: ${discount_label}`
			);
		}
	}
